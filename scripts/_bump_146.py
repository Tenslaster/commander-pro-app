#!/usr/bin/env python3
"""Bump Commander PRO to 1.4.6 (gestion granular rights + OWNER-only notify)."""
...
def main() -> None:
    ...
    if pol_path.is_file():
        ...
        pol["min_app_version"] = "1.4.5"
        pol["latest_app_version"] = VER
        pol["latest_app_version_android"] = VER
        pol["latest_app_version_ios"] = VER
        pol["force_update"] = True
        pol["message_fr"] = (
            f"Mettez a jour vers {VER} (gestion droits detailles + securite OWNER)."
        )
        pol["message_en"] = (
            f"Please update to {VER} (detailed gestion rights + OWNER security)."
        )
        pol["soft_message_fr"] = f"Commander PRO {VER}: droits gestion detailles."
        pol["soft_message_en"] = f"Commander PRO {VER}: detailed gestion rights."
