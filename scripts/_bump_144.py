#!/usr/bin/env python3
"""Bump Commander PRO to 1.4.4 (stats + gestion rights)."""
...
def main() -> None:
    ...
    if pol_path.is_file():
        ...
        pol["min_app_version"] = "1.4.3"
        pol["latest_app_version"] = VER
        pol["latest_app_version_android"] = VER
        pol["latest_app_version_ios"] = VER
        pol["force_update"] = True
        pol["message_fr"] = f"Mettez a jour vers {VER} (stats + droits gestion)."
        pol["message_en"] = f"Please update to {VER} (stats + manage rights)."
        pol["soft_message_fr"] = f"Commander PRO {VER}: stats + permissions."
        pol["soft_message_en"] = f"Commander PRO {VER}: stats + permissions."
