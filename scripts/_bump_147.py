#!/usr/bin/env python3
"""Bump Commander PRO to 1.4.7 (offline false-positive fix + stats)."""
...
def main() -> None:
    ...
    if pol_path.is_file():
        ...
        pol["min_app_version"] = "1.4.6"
        pol["latest_app_version"] = VER
        pol["latest_app_version_android"] = VER
        pol["latest_app_version_ios"] = VER
        pol["force_update"] = True
        pol["message_fr"] = f"Mettez a jour vers {VER} (connexion + stats)."
        pol["message_en"] = f"Please update to {VER} (connection + stats)."
        pol["soft_message_fr"] = f"Commander PRO {VER}: moins de faux offline."
        pol["soft_message_en"] = f"Commander PRO {VER}: fewer false offline."
